...
export async function adminWorkEditView(params) {
  ...
  const yearI = h('input', { type: 'number', value: existing?.year || new Date().getFullYear() });
  ...
  const toggleImages = () => { imagesSection.hidden = typeSel.value === 'comic'; };
  typeSel.addEventListener('change', toggleImages);
  toggleImages();
  ...
  const form = h('form', { class: 'form', on: { submit: async (e) => {
    ...
    const payload = {
      ...
      year: Number(yearI.value) || new Date().getFullYear(),
    ...
    h('div', { class: 'field' }, [h('label', { class: 'field__label' }, '排序权重（数字越小越靠前）'), sortI]),
